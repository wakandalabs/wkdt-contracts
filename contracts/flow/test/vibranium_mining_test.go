...
type TestVibraniumMiningContractsInfo struct {
	FTAddr          flow.Address
...
func VibraniumMiningDeployContract(b *emulator.Blockchain, t *testing.T) TestVibraniumMiningContractsInfo {
	accountKeys := test.AccountKeyGenerator()

	bpInfo := WakandaPassDeployContract(b, t)

	vibraniumMiningAccountKey, vibraniumMiningSigner := accountKeys.NewWithSigner()
	vibraniumMiningCode := loadVibraniumMining(bpInfo)

	vibraniumMiningAddr, err := b.CreateAccount(
		[]*flow.AccountKey{vibraniumMiningAccountKey},
		[]templates.Contract{{
			Name:   "VibraniumMining",
			Source: string(vibraniumMiningCode),
		}},
	)
	...
	return TestVibraniumMiningContractsInfo{
		FTAddr:          bpInfo.FTAddr,
		NFTAddr:         bpInfo.NFTAddr,
		VAddr:          bpInfo.VAddr,
		VSigner:        bpInfo.VSigner,
		VStakingAddr:   bpInfo.VStakingAddr,
		VStakingSigner: bpInfo.VStakingSigner,
		WPAddr:          bpInfo.WPAddr,
		WPSigner:        bpInfo.WPSigner,
		BTMiningAddr:    vibraniumMiningAddr,
		BTMiningSigner:  vibraniumMiningSigner,
	}
}

func VibraniumMiningSetupMiningReward(
	t *testing.T, b *emulator.Blockchain, btMiningInfo TestVibraniumMiningContractsInfo,
...
func TestBPMiningDeployment(t *testing.T) {
	...
	btMiningInfo := VibraniumMiningDeployContract(b, t)
...
func TestBPMiningOneRound(t *testing.T) {
	...
	// Add Vibranium minter
	SetupVibraniumMinterForStaking(
		t, b,
		btMiningInfo.FTAddr, CadenceUFix64("10000.0"),
		btMiningInfo.VAddr, btMiningInfo.VSigner,
		btMiningInfo.BTMiningAddr, btMiningInfo.VSigner)
	...
	assert.NoError(t, err)
	// Add Wakanda Pass
	MintNewWakandaPass(t, b, btMiningInfo.NFTAddr, user1Addr, user1Signer, btMiningInfo.WPAddr, btMiningInfo.WPSigner)
	VibraniumMiningSetupMiningReward(t, b, btMiningInfo, user1Addr, user1Signer)
	...
	assert.NoError(t, err)
	// Add Wakanda Pass
	MintNewWakandaPass(t, b, btMiningInfo.NFTAddr, user2Addr, user2Signer, btMiningInfo.WPAddr, btMiningInfo.WPSigner)
	// TODO: user2 has WakandaPass on VIP tier 1
	VibraniumMiningSetupMiningReward(t, b, btMiningInfo, user2Addr, user2Signer)
	...
	t.Run("Should be able to collect data with VIP-tier Wakanda Pass", func(t *testing.T) {
		tx := flow.NewTransaction().
			SetScript(btMiningCollectDataTransaction(btMiningInfo.BTMiningAddr)).
			SetGasLimit(100).
			SetProposalKey(b.ServiceKey().Address, b.ServiceKey().Index, b.ServiceKey().SequenceNumber).
...
func loadVibraniumMining(bpInfo TestWakandaPassContractsInfo) []byte {
	code := string(readFile(vibraniumMiningPath))
	...
	code = strings.ReplaceAll(code, "\"../token/Vibranium.cdc\"", "0x"+bpInfo.VAddr.String())
	code = strings.ReplaceAll(code, "\"../token/WakandaPass.cdc\"", "0x"+bpInfo.WPAddr.String())
...
func btMiningGetPropertyScript(filename string, btMiningAddr flow.Address) []byte {
	return []byte(strings.ReplaceAll(
		...
		"\"../../contracts/flow/mining/VibraniumMining.cdc\"",
